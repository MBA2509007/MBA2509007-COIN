...
let client = null;
let isDbReady = false;
let dbError = "Initiating Apex Terminal...";
...
async function connectToDb() {
    ...
    try {
        ...
        // 自动迁移与 Admin 初始化
        try { await client.query('ALTER TABLE users ALTER COLUMN balance TYPE BIGINT'); } catch(e){}
        
        // 【关键】Admin 账户逻辑：如果不存在则创建，如果存在则确保 PIN 是 888888
        ...
        isDbReady = true;
        console.log("MBA_SYSTEM: Admin Vault Secured (PIN: 888888)");
    } catch (err) {
        dbError = err.message;
...
<head>
    ...
    <title>MBA2509007 | COIN TERMINAL</title>
    <link href="https://fonts.googleapis.com/css2?family=Exo+2:wght@400;700&family=JetBrains+Mono&display=swap" rel="stylesheet">
    <style>
        :root { --gold: #f0b90b; --bg: #050505; --panel: rgba(10, 12, 16, 0.9); --border: #1A1C22; --text: #E0E2E5; --glow: rgba(240, 185, 11, 0.4); }
        body { background: var(--bg); color: var(--text); font-family: 'Exo 2', sans-serif; margin: 0; overflow: hidden; height: 100vh; }
        .header { position: fixed; top: 0; width: 100%; height: 60px; background: rgba(0,0,0,0.85); backdrop-filter: blur(15px); border-bottom: 1px solid var(--border); display: flex; justify-content: center; align-items: center; z-index: 100; }
        ...
        .sidebar { width: 420px; background: var(--panel); border-left: 1px solid var(--border); padding: 30px; display: flex; flex-direction: column; z-index: 10; box-shadow: -20px 0 50px rgba(0,0,0,0.9); backdrop-filter: blur(15px); }
        .card { background: rgba(20, 20, 25, 0.6); border: 1px solid var(--border); padding: 20px; border-radius: 12px; margin-bottom: 20px; transition: 0.3s; }
        .card:hover { border-color: #333; box-shadow: 0 0 15px rgba(240,185,11,0.05); }
        ...
        .tab-btn { flex: 1; padding: 12px; background: #111; color: #555; border: none; border-radius: 6px; cursor: pointer; font-size: 11px; font-weight: 700; transition: 0.3s; }
        ...
        #bal-view { display: none; text-align: center; margin-top: 5px; animation: slideIn 0.4s ease; }
        @keyframes slideIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
    </style>
</head>
<body>
    ${isDbReady ? content : '<div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#f0b90b;background:#000;"><div style="width:40px;height:40px;border:3px solid #111;border-top-color:#f0b90b;border-radius:50%;animation:spin 1s linear infinite;margin-bottom:20px;"></div><div style="letter-spacing:5px;font-weight:700;">APEX NODE INITIALIZING...</div><style>@keyframes spin{to{transform:rotate(360deg)}}</style></div>'}
    <script>
        // 地图增强版引擎 (V13.8)
        ...
        function init(){
            ...
            pts=[];
            for(let i=0; i<450; i++){
                // 模拟地理分布：通过对经纬度进行加权，模拟大陆板块密集效果
                let t=Math.random()*Math.PI*2; // 经度
                let a;
                if(Math.random() > 0.4) {
                    a = Math.acos(Math.random()*1.4 - 0.7); // 偏向赤道和中纬度（模拟大陆密集区）
                } else {
                    a = Math.acos(Math.random()*2 - 1); // 随机分布
                }
                pts.push({
                    x: Math.sin(a)*Math.cos(t), y: Math.sin(a)*Math.sin(t), z: Math.cos(a),
                    isCap: Math.random() > 0.92 // 核心城市节点
                });
            }
        }
        let rot=0;
        function draw(){
            ...
            rot+=0.0025; x.save(); x.translate(w/2, h/2);
            let active = pts.map(p => {
                let x1=p.x*Math.cos(rot)-p.z*Math.sin(rot), z1=p.z*Math.cos(rot)+p.x*Math.sin(rot);
                return { x: x1*Math.min(w,h)*0.38, y: p.y*Math.min(w,h)*0.38, z: z1, cap: p.isCap };
            });
            // 连线逻辑
            x.lineWidth=0.6;
            for(let i=0; i<active.length; i++){
                if(active[i].z < -0.2) continue; // 不渲染背面的线
            ...
            active.forEach(p => {
                ...
                x.fillStyle = p.cap ? \`rgba(255,255,255,\${s})\` : \`rgba(240,185,11,\${0.7*s})\`;
                if(p.cap) { x.shadowBlur=15*s; x.shadowColor="#fff"; }
    ...
    try {
        ...
        res.send(getLayout(`
            ...
            <div class="container">
                ...
                <div class="sidebar">
                    <div class="tabs">
                        <button class="tab-btn active" id="t1" onclick="sw('tx')">SHIPMENT</button>
                        <button class="tab-btn" id="t2" onclick="sw('rg')">NEW VAULT</button>
                    </div>
                    <div id="b-tx" class="card">
                        <input type="text" id="f" placeholder="SENDER ID (e.g. Admin)">
                    ...
                    <div id="bal-view" class="card" style="border-color:#f0b90b; background: rgba(240,185,11,0.05);">
                        <div style="font-size:10px;color:#666;text-transform:uppercase;letter-spacing:1px;">VAULT: <span id="v-u" style="color:#fff"></span></div>
                        <div id="v-n" style="font-size:2.8rem;font-weight:700;color:#fff;margin:5px 0;">0</div>
                        <div style="font-size:10px;color:#f0b90b;letter-spacing:3px;font-weight:700;">COIN ASSETS</div>
                    </div>
                    <div style="font-size:10px;color:#333;margin-bottom:10px;letter-spacing:1px;font-weight:700;">SYSTEM_LEDGER_LIVE</div>
                    <div class="log-list" style="overflow-y:auto;flex:1;">${logHtml || '<div style="color:#222;font-size:10px;">AWAITING NETWORK ACTIVITY...</div>'}</div>
                </div>
            </div>
        `));
...
app.get('/api/reg', async (req, res) => {
    try { await client.query('INSERT INTO users (name, balance, pin) VALUES ($1, 0, $2)', [req.query.u, req.query.p]); res.send("<script>alert('Identity Registered');location.href='/';</script>"); }
    catch (e) { res.send("<script>alert('ID Already Exists');location.href='/';</script>"); }
});
...
app.get('/api/pay', async (req, res) => {
    ...
    try {
        ...
        if(auth.rows.length === 0) return res.send("<script>alert('AUTHENTICATION FAILED: Invalid ID or PIN');location.href='/';</script>");
        const dec = await client.query('UPDATE users SET balance = balance - $1 WHERE name = $2 AND balance >= $1', [amt, f]);
        if(dec.rowCount === 0) return res.send("<script>alert('TRANSACTION DENIED: Insufficient COIN');location.href='/';</script>");
