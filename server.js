...
let client = null;
let isDbReady = false;
let dbError = "Initializing Quantum Node...";
...
<head>
    ...
    <style>
        :root { --gold: #f0b90b; --bg: #050505; --panel: rgba(13, 13, 13, 0.92); --border: #222; }
        ...
        .supply-tag { font-family: 'Orbitron'; font-size: 14px; color: var(--gold); letter-spacing: 3px; }
        ...
        .card { background: #0a0a0a; border: 1px solid var(--border); padding: 22px; border-radius: 12px; margin-bottom: 25px; }
        input { width: 100%; padding: 14px; margin-bottom: 12px; background: #000; border: 1px solid #222; color: var(--gold); border-radius: 6px; box-sizing: border-box; font-family: inherit; outline: none; }
        ...
        .tab-btn { background: #1a1a1a; color: #444; border: none; padding: 12px; cursor: pointer; font-family: 'Orbitron'; font-size: 10px; flex: 1; border-radius: 4px; margin: 0 5px; }
        .tab-btn.active { background: var(--gold); color: #000; }
    </style>
</head>
<body>
    ${isDbReady ? content : '<div style="display:flex;justify-content:center;align-items:center;height:100vh;color:#f0b90b;font-family:Orbitron;">SYNCHRONIZING...</div>'}
</body>
</html>`;
...
app.get('/', async (req, res) => {
    ...
    let logHtml = logs.rows.map(l => `<div class="log-item"><span>${l.sender} → ${l.receiver}</span><b style="color:#f0b90b;">+${l.amount.toLocaleString()} COIN</b></div>`).join('');

    res.send(getLayout(`
        <div class="header"><div class="supply-tag">RESERVE: ${total.toLocaleString()} COIN</div></div>
        <div class="container">
...
app.get('/api/reg', async (req, res) => {
    ...
    catch (e) { res.send("<script>alert('Conflict.');location.href='/';</script>"); }
});
...
app.get('/api/bal', async (req, res) => {
    ...
    res.send(getLayout(`<div style="display:flex;justify-content:center;align-items:center;height:100vh;"><div class="card" style="width:350px;text-align:center;border-color:#f0b90b;"><div style="font-family:Orbitron;color:#f0b90b;font-size:12px;">ID: ${req.query.u}</div><div style="font-size:3.5rem;margin:20px 0;font-family:Orbitron;text-shadow:0 0 20px #f0b90b;">${b.toLocaleString()}</div><div style="color:#f0b90b;font-size:10px;margin-bottom:30px;letter-spacing:3px;">COIN CREDITS</div><button class="btn btn-gold" onclick="location.href='/'">BACK</button></div></div>`));
});
