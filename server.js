...
async function startServer() {
    try {
        ...
        // 自动平账：确保除了 Admin，其他人的币都是 0，Admin 拥有 1,000,000
        ...
        console.log("✅ TOTAL SUPPLY SECURED AT 1,000,000 WOW.");
...
<head>
    ...
    <style>
        ...
        .log-box { margin-top: auto; height: 150px; font-size: 11px; color: #666; overflow-y: auto; border-top: 1px solid #333; padding-top: 10px; }
    </style>
</head>
<body>
`;
    ...
    <div class="wrapper">
        ...
        <div class="right-zone">
            <div class="terminal-header">
                <img src="/coin.jpg" class="coin-img" onerror="this.src='https://cryptologos.cc/logos/dogecoin-doge-logo.png'">
                <h2 style="font-family:'Orbitron'; color:var(--bronze); margin:0;">GLOBAL TERMINAL</h2>
            </div>
            <button class="btn btn-check" onclick="check()">🔍 ACCESS VAULT</button>
            <div class="panel">
                <input type="text" id="f" placeholder="Sender">
                <input type="text" id="t" placeholder="Receiver">
                <input type="number" id="a" placeholder="Amount">
                <button class="btn btn-send" onclick="send()">🚀 EXECUTE TRANSFER</button>
            </div>
            <div class="log-box"><b>LOGS:</b><br>${logHtml}</div>
        </div>
    </div>
    <script>
        ...
        let ay = 0;
        function draw() {
            ctx.fillStyle = '#000'; ctx.fillRect(0,0,width,height); const r = Math.min(width, height)*0.35; ay += 0.003;
            ctx.save(); ctx.translate(width/2, height/2);
            nodes.forEach(p => {
                let x1 = p.x*Math.cos(ay) - p.z*Math.sin(ay); let z1 = p.z*Math.cos(ay) + p.x*Math.sin(ay);
                ctx.fillStyle = "rgba(201,156,84," + (z1+1)/2 + ")"; ctx.beginPath();
                ctx.arc(x1*r, p.y*r, (z1+1.5)*1.5, 0, Math.PI*2); ctx.fill();
            });
    ...
    if (r.rows.length === 0) {
        ...
        <div style="display:flex;justify-content:center;align-items:center;height:100vh;">
            <div class="panel" style="width:350px; text-align:center; border: 2px solid var(--bronze);">
                <img src="/coin.jpg" style="width:80px; border-radius:50%;">
                <h3 style="color:var(--bronze);">${name} VAULT</h3>
                <div style="font-size:50px; font-family:'Orbitron'; color:var(--gold);">${r.rows[0].balance}</div>
...
app.get('/api/pay', async (req, res) => {
    ...
    try {
        ...
        if (s.rowCount === 0) return res.send("Denied.");
